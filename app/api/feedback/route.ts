...
export async function GET() {
  try {
    ...
    console.error(
      "Unexpected feedback fetch error:",
      error
    );
...
export async function POST(request: Request) {
  try {
    ...
    const authUserId =
      body.auth_user_id?.trim() || null;
    const avatarUrl =
      body.avatar_url?.trim() || null;
    const role = body.role?.trim();
    const message = body.message?.trim();
    const cardColor =
      body.cardColor?.trim() || "purple";
    ...
    if (!authUserId || !email) {
      return NextResponse.json(
        {
          error:
            "You must sign in before submitting feedback.",
        },
        {
          status: 401,
        }
    ...
    if (role && role.length > 80) {
      return NextResponse.json(
        {
          error:
            "Role must be 80 characters or fewer.",
        },
        {
          status: 400,
        }
    ...
    const safeColor = allowedColors.includes(
      cardColor
    )
      ? cardColor
      : "purple";
